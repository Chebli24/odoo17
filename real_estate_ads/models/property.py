...
class Property(models.Model):
    ...
    _inherit =['mail.thread', 'mail.activity.mixin', 'website.published.mixin' ] #mixins
    ...
    expected_price = fields.Monetary(string="Expected Price", tracking=True) # active tracking of changes shown in the mail messaging model
    ...
    offer_ids = fields.One2many('estate.property.offer', 'property_id', string="Offer") #the second argument is so that we can link property_id to offer_id
    ...
    def _compute_best_price(self):
        for rec in self:
            if rec.offer_ids:
                rec.best_offer = max(rec.offer_ids.mapped('price')) #sets the best offer field to the highest price
            else:
                rec.best_offer = 0
